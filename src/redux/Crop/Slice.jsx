...
//Get all crop info
export const fetchCropData = createAsyncThunk('crop/getAllCropsInfo', async (_, { rejectWithValue, getState }) => {
  try {
...
export const updateCrop = createAsyncThunk('crop/updateCrop', async (cropId, { getState, dispatch }) => {
  try {
    if (ethereum) {
      dispatch(setLoadingState(true));
      const state = getState();
      ...
      const { cropType, plantingDate, harvestDate, fertilizers, pesticides, diseases, additionalInfo } =
        state.crop.updateCrop.updateFormData;
      ...
      const cropHash = await cropInfoContract.updateCropInfo(
        cropId,
        cropType,
        // unixPlantingDate,
        harvestDate,
...
export const Slice = createSlice({
  ...
  reducers: {
    ...
    setUpdateFormData: (state, action) => {
      const { name, value } = action.payload;
      if (name in state.formData) {
        state.updateCrop.updateFormData[name] = value;
      } else {
        console.warn(`Tried to set formData with unknown field: ${name}`);
      }
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateCrop.fulfilled, (state, action) => {
        state.updateCrop.status = 'completed';
        // state.updateCrop.data = action.payload
      })
      .addCase(updateCrop.pending, (state) => {
        state.updateCrop.status = 'loading';
      })
      .addCase(updateCrop.rejected, (state, action) => {
        state.updateCrop.status = 'rejected';
        state.updateCrop.error = action.error.message;
        // state.updateCrop.data = null
      })

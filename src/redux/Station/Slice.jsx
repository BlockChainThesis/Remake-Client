...
export const getStationData = createAsyncThunk(
    'station/getStationData', async (stationId,{dispatch}) => {
        try{
            if(ethereum) {
                dispatch(setLoadingState(true))
                const contract  = createContract ()
...
export const getSensorHistory = createAsyncThunk(
    'station/getSensorHistory', async ({stationId, sensorId, length},{dispatch}) => {
        try{
            if(ethereum) {
                dispatch(setLoadingState(true))
                const contract = createContract()
                const sensorHistory = await contract.getHistorySensorValue(stationId, sensorId,length)
                const structuredTime = sensorHistory[0].map((time) => 
...
export const getNumberofStations = createAsyncThunk(
    'station/getNumberofStations', async (_,{dispatch}) => {
        try{
            if(ethereum) {
                dispatch(setLoadingState(true))
                const contract = createContract()

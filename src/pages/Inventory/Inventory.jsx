...
const Inventory = () => {
  ...
  return (
    <>
      {modalState.isOpen && (
        <InventoryModal data={modalState.data} type={modalState.type} setModalState={setModalState} />
      )}
              ...
              {cropInfo.length === 0 ? (
                ...
                filteredResult.crop.map((item, index) => (
                  <div key={index} className="group cursor-pointer" onClick={() => handleItemClick('crop', item)}>
                    <div
                      className="flex rounded bg-main-100 group-hover:bg-main-400 font-mono
              ...
              {unListedNFT.length === 0 ? (
                ...
                filteredResult.nft.map((item, index) => (
                  <div key={index} className="group cursor-pointer" onClick={() => handleItemClick('inventory', item)}>
                    <div
                      className="flex rounded bg-main-100 group-hover:bg-main-400
                    ...
                    >
                      ...
                      <img
                        className="w-[40px] h-[35px] object-cover rounded border border-main-300"
                        src={urlDN + item.uri}
                      />
                    </div>
                  </div>
        ...
        {filters.listed && (
          <Window label="On Market">
            <div className="grid grid-cols-2 gap-x-2 gap-y-3 p-4">
              {listedNFT.length === 0 ? (
                <p className="text-main-100 font-semibold col-span-2">No item on sale :(</p>
              ) : (
                filteredResult.listed.map((item, index) => (
                  <div key={index} className="group cursor-pointer" onClick={() => handleItemClick('sale', item)}>
                    ...
                    >
                      <p className="font-bold">{item.tokenId}.</p>
                      <img
                        className="w-[40px] h-[40px] object-cover rounded border border-main-300"
                        src={urlDN + item.uri}
                      />
                      <div className="group-hover:text-main-100">
                        <h1 className="tablet:text-lg text-sm font-bold tracking-wide">{item.crop.cropType}</h1>
                        <p className="font-semibold text-xs">{item.price === 0 ? 'Not pricing' : item.price + 'FLP'}</p>
                      </div>
                    </div>
                  </div>
                ))
